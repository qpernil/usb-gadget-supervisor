...
| --- | --- | --- | --- |
| `BIND` | Function attached to a ConfigFS gadget | forward `Bind`; keep pumps disabled | reset virtual controller; remain unconfigured |
| `UNBIND` | Gadget/function generation removed | forward `Unbind`; stop endpoint use | reset virtual controller |
| `ENABLE` | Host selects configuration or an interface altsetting is activated | enable endpoint pumps; forward `Enable` | configure firmware once; accept packets |
| `DISABLE` | Host selects configuration zero, bus reset, cable disconnect, or gadget disable | disable pumps; forward `Disable` | discard endpoint state and reset |
...
| `SUSPEND` | Host suspends the configured USB link, commonly during host sleep | forward only; leave generation and pumps intact | preserve state and invoke suspend callback |
...
The worker process, USB generation, descriptor personality, endpoint proxies,
...
starts a new worker. Generation replacement unbinds the UDC, stops and joins
all old endpoint pumps, closes old FunctionFS files, and then creates an
entirely new ConfigFS/FunctionFS projection and set of proxies.
